  ...
  build:
    ...
    steps:
      ...
      - run: |
          ...
          rm -rf "$OUT"
          mkdir -p "$OUT"
          # Создаём _config.yml, чтобы Jekyll понимал, что md — это просто папка с файлами
          echo "include: [md]" > "$OUT/_config.yml"
            ...
            if [[ -d "$repo_dir" ]]; then
              ...
              find "$repo_dir" -mindepth 1 -maxdepth 1 -not -path "$repo_dir/.git"|while read -r item;do
                ...
                if [[ -d "$item" ]];then
                  ...
                  # Файл: ссылка ведёт на сам файл (без слеша)
                  echo "- [$rel_path](./files/main/$rel_path)" >> "$md_repo/index.md"
                fi
              done
              # Рекурсивный обход всех элементов (папки и файлы)
              find "$repo_dir" -not -path "$repo_dir/.git/*" -not -path "$repo_dir/.git" -type f -o -type d|while read -r item;do
                ...
                if [[ -d "$item" ]]; then
                  ...
                  # Перечисляем элементы внутри папки
                  find "$item" -mindepth 1 -maxdepth 1|while read -r child;do
                    ...
                    if [[ -d "$child" ]];then
                      ...
                      # Файл: ссылка на файл (без слеша)
                      echo "- [$child_name](./$child_name)" >> "$target_dir/index.md"
                    fi
                  done
                else
                  # Это файл — создаём .md-файл прямо в папке, а не подпапку
                  file_path="$files_main/$rel_path"
                  # Проверяем, не существует ли уже папка с таким именем
                  if [[ -d "$file_path" ]]; then
                    # Если папка существует, пропускаем (это уже обработано как директория)
                    continue
                  fi
                  # Создаём .md-файл
